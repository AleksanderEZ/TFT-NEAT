...
function GetSpritesCenterOnly()
	if gameinfo.getromname() == ROMName1942 then
		local sprites = {}
		for i = SPRITE_BUFFER_START, SHIP_ADDRESS_START - 1, 20 do
			if memory.readbyte(i + 5) ~= 0xFF then
				local enemyY = memory.readbyte(i + 4)
				local enemyX = memory.readbyte(i + 7)
				sprites[#sprites + 1] = { ["x"] = enemyX,["y"] = enemyY }
			end
			print(sprites[#sprites])
		end
		for i = SHIP_ADDRESS_END + 1, SPRITE_BUFFER_END, 20 do
			if memory.readbyte(i + 5) ~= 0xFF then
				local enemyY = memory.readbyte(i + 4)
				local enemyX = memory.readbyte(i + 7)
				sprites[#sprites + 1] = { ["x"] = enemyX,["y"] = enemyY }
...
function GetSprites()
	if gameinfo.getromname() == ROMName1942 then
		local sprites = {}
		for i = SPRITE_BUFFER_START, SHIP_ADDRESS_END, 4 do
			local sprite = memory.readbyte(i + 1)
			if sprite ~= 0xFF then
				local spriteX = memory.readbyte(i)
				local spriteY = memory.readbyte(i + 3)
...
function GetInputs()
	...
	for Y = 0, POV_HEIGHT, 1 do
		for X = 0, POV_WIDTH, 1 do
			inputs[#inputs + 1] = 0

			for i = 1, #sprites do
				if X == sprites[i]["x"] then
					if Y == sprites[i]["y"] then
						inputs[#inputs] = -1
					end
				end
			end
...
function DisplayGenome(genome)
	...
	local i = 1
	local cell = {}
	for dy = -240 / 2, 240 / 2, 1 do
		for dx = -255 / 2, 255 / 2, 1 do
			cell = {}
			cell.x = 50 + 5 * dx
			cell.y = 70 + 5 * dy
	...
	for n, cell in pairs(cells) do
		if n > INPUTS or cell.value ~= 0 then
			local color = 0
			if cell.value == 1 then color = 255 end
			if cell.value == -1 then color = 0 end
			color = color * 0x10000 + color * 0x100 + color
