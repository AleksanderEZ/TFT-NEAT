...
function GetDistanceToDoor()
	local distanceX = math.abs(DoorX - BombermanX)
	local distanceY = math.abs(DoorY - BombermanY)
	local distanceToDoor = distanceX + distanceY
	return distanceToDoor * distanceToDoor
end
...
while true do
	...
	if IsDead or hasTimedOut then
		local fitness = 100000000 / GetDistanceToDoor() - Pool.currentFrame/2
	...
	if not forms.ischecked(HideBanner) then
		gui.drawText(0, 0,
			...
			0xFF000000, 11)
		gui.drawText(0, 12, "Fitness: " .. math.floor(100000000 / GetDistanceToDoor()),
			0xFF000000, 11)
